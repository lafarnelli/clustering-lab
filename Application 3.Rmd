...
```
Question 1: Analyzing the plants data using PAM clustering: 
```{r}
...
```
I chose 2 clusters because that k value yields a higher average silhouette width (.72, closest to 1) of all the options between k=2 and k=10. Let's consider cluster 2:
```{r}
colMeans(X[pm$clustering == 2,])
```
Question 2: 
```{r}
hc <- hclust(D)
